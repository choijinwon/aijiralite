  ...
  if (authOptions) {
    try {
      ...
      if (session?.user?.id) {
        ...
        if (user) {
          console.log('✅ [AUTH] Authenticated via NextAuth session:', {
            id: user.id,
            email: user.email,
            sessionEmail: session.user.email
          });
          // Ensure we return the correct email from database, not from session
          return user;
        } else {
          console.warn('⚠️ [AUTH] User not found in DB for session ID:', session.user.id);
        }
        ...
        if (user) {
          console.log('✅ [AUTH] Authenticated via NextAuth session (by email):', {
            id: user.id,
            email: user.email,
            sessionEmail: session.user.email
          });
          ...
          console.warn('⚠️ [AUTH] User not found in DB for session email:', session.user.email);
        }
      }
    } catch (error) {
